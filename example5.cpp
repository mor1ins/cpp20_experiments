...
template <size_t N, const char (&Data)[N]>
...
template <size_t N>
consteval auto
get_delimiter(const char (&delimiter)[N]) {
...
consteval auto get_reverse_range(auto end) {
    return std::ranges::views::reverse(std::ranges::views::iota(0, static_cast<int>(end + 1)));
}
...
public:
    ...
    consteval auto get_times() const {
        return get_reverse_range(this->time);
    }
